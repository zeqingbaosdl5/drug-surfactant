...
drug_file_dict = {
    ...
    'DCF': 'diclofenac.json',
    'LOV': 'lovastatin.json',
    'ITZ': 'itraconazole.json',
    'RPD': 'risperidone.json',
    'GLV': 'griseofulvin.json',
    'CTZ': 'clotrimazole.json',
    'GBC': 'glibenclamide.json',
}
...
def run_optimizer(current_iteration, drug_list, bopt, n_trials=1): # modified to run separate optimizers for each drug
    ...
    for drug in drug_list:
        ax_client = AxClient.load_from_json_file(drug_path + drug_file_dict[drug])
        ...
        # 9. 保存状态
        ax_client.save_to_json_file(drug_path + drug_file_dict[drug])
        df_design.to_csv(f"{design_file_path}i{current_iteration}.csv", index=False)

        df_design_dict[drug] = df_design
        ax_client_dict[drug] = ax_client
        data_so_far_dict[drug] = data_so_far
        best_concs_dict[drug] = best_concs
    
    return df_design_dict, ax_client_dict, data_so_far_dict, best_concs_dict
