...
def optimizer_init():
    ...
    ax_client.create_experiment(
        ...
        objectives={
            'micelle_drug_conc': ObjectiveProperties(minimize=False, threshold=0.1),
            'success': ObjectiveProperties(minimize=False, threshold=0.5),  # success rate threshold
            'initial_drug_conc_surfactant_conc_ratio': ObjectiveProperties(minimize=False),
...
def normalize_data(df, mode):
    ...
    factors = {
        'initial_drug_conc_surfactant_conc_ratio': 100,  # normalized value is between 0 and 1
...
def run_optimizer(current_iteration, drug, bopt, n_trials):
    ...
    drug_features = ObservationFeatures(parameters = normalize_drug_properties_dict[drug]["normalized_properties"])
        ...
        trials_data.append(
            {
                ...
                **parameters,
                ...
                "success": None,
                "initial_drug_conc_surfactant_conc_ratio": None, 
                "Complexity": None,
...
def load_data_to_optimizer(iteration, norm_results):
    ...
    for _, row in labeled_data.iterrows():
        ...
        raw_data = {
            ...
            "success": (row["success"]),
            "initial_drug_conc_surfactant_conc_ratio": (row["initial_drug_conc_surfactant_conc_ratio"]),
