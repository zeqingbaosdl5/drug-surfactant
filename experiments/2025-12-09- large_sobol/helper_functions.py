...
def optimizer_init():
    ...
    ax_client.create_experiment(
        ...
        parameters=[
            ...
            #{"name": "surf_1_conc", "type": "range", "bounds": [0.0, surfactant_stock_conc], "value_type": "int"}, # original
            {"name": "surf_1_conc", "type": "range", "bounds": [1, 10], "value_type": "int"}, # made lowest be 1, max=20

            {"name": "surf_2", "type": "choice", "is_ordered": False, "values": ["s1", "s2", "s3", "s4", "s5", "s6", "s7", "s8"]},
            #{"name": "surf_2_conc", "type": "range", "bounds": [1.0, surfactant_stock_conc], "value_type": "int"}, #original
            {"name": "surf_2_conc", "type": "range", "bounds": [1, 10], "value_type": "int"}, #made lowest be 1, max 20
        ...
        parameter_constraints=[
            ...
            f"surf_1_conc + surf_2_conc  <= {10-1}",
    ...
    for drug in drug_list:
       ...
       # if current_iteration > 0 and isinstance(best_conc, (int, float)):
        ...
        if best_concs is not None and isinstance(best_conc, (int, float)):
            upper_bound = math.floor(best_conc * 0.9)
            new_constraints = [
            # 上界：surf_1_conc + surf_2_conc <= best_conc - 1
            SumConstraint(parameters=param_objs, is_upper_bound=True,  bound=upper_bound),
            # 下界：surf_1_conc + surf_2_conc >= 1
            SumConstraint(parameters=param_objs, is_upper_bound=False, bound=1),
        ]
        
            space._parameter_constraints = new_constraints
            print(f"Update constraints (drug={drug})：surf_1_conc+surf_2_conc <= {upper_bound}，>=1")
        ...
        for _ in range(n_trials):
            ...
            ############################New part to this branch (forces surf_1 != surf_2)
            # surf_1 must NOT equal surf_2
            #    if parameters["surf_1"] != parameters["surf_2"]:
             #       break  # ✅ valid trial → accept it

            # invalid → discard & resample  
              #  ax_client.abandon_trial(trial_index)
               # print(
                #    f"Rejected trial (drug={drug}): "
                 #   f"surf_1 == surf_2 == {parameters['surf_1']}"
                #)
