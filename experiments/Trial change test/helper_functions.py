...
def change_loaded_data_to_fail(iteration, trial_index):

    n= iteration
    folder = f"iteration_{iteration}"
    base_path = os.path.join(folder, 'optimizer', f'optimizer_{iteration}')

    json_file = base_path + '.json'
    save_path = base_path + '_loaded.json'
    ...
    ax_client.complete_trial(trial_index=trial_index, raw_data=raw_data)
    ax_client.save_to_json_file(path)

    print(f"Trial {trial_index} in iteration {iteration} marked as success=0, surfactant_input=1, complexity=1")
