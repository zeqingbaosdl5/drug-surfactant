...
def setup_client(host, username, password, device_id="otflex_001"):
    """
    Setup the MQTT client for OT-Flex orchestrator.
    ...
    return client, queue, connected_event, command_topic, data_topic
...
client, queue, connected_event, command_topic, data_topic = setup_client(
    host, username, password, device_id
)
...
if not connected_event.wait(timeout=10.0):
    raise ConnectionError("OT-Flex Orchestrator connection timeout - failed to connect to MQTT broker")
